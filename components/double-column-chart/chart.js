...
function drawYAxis(opts, config, context) {
    ...
    context.save();
    context.beginPath();
    var grd = context.createLinearGradient(0, 0, yAxisWidth, yAxisHeight);
    ...
    if (config.yAxis.rightData != undefined && config.yAxis.rightData != null && config.yAxis.rightData.length > 1) {
        ...
        for (var i = 0; i < rightDataLength; i++) {
            var _axis = {};
            _axis.x = config.canvasWidth - config.yAxis.dataWidth;
            _axis.y = config.yAxis.padd * (config.yAxis.maxData - config.yAxis.rightData[i].y) + padd.top + axisMargin.top + config.axisPadd.top + config.yAxis.fontSize / 2;
            _axis.content = config.yAxis.rightData[i].title;
            yRightAxis.push(_axis);
        }
        ...
        yRightAxis.forEach(function (item) {
            var metricsContent = context.measureText(item.content).width;
            var rigthX = item.x + config.yAxis.dataWidth - metricsContent;
...
function drawColumn(series, opts, config, context) {
    ...
    areaDatas.forEach(function (item) {
        var grd = context.createLinearGradient(item.x, item.y, item.x, startPoint.y);
        ...
        lines.forEach(function (items, index) {
            ...
            if (items.point.isShow) {
                items.data.forEach(function (item) {
                    context.beginPath();
                    var bgrd = context.createLinearGradient(0, 0, 1, 1);
                    ...
                    context.beginPath();
                    var sgrd = context.createLinearGradient(0, 0, 1, 1);
    ...
    if (config.touchDetail.isShow) {
        ...
        context.beginPath();
        var bgColorGrd = context.createLinearGradient(0, 0, 1, 1);
